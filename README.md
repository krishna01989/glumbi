...
| 🔦 **Torch Hunt** | A dark-arena hidden-object game. The arena is pitch-black; the child moves a torch (flashlight) around the canvas to find a target object. Hovering the torch over the target triggers a dwell countdown — hold steady to reveal it and unlock a fun fact. Object count, torch radius, and dwell speed all adapt to the child's age. Supports touch and desktop. |
...
| 🚦 **Signup Kill Switch** | Admins can temporarily pause new user registrations (email + Google OAuth) without affecting existing users. Toggle on the admin dashboard; state persisted in `AppSetting` with a 30s in-memory cache. Returns `503` with `Retry-After: 86400` and `code: SIGNUP_PAUSED` so monitoring tools and the frontend can distinguish it from a real outage. The auth page proactively fetches `/api/auth/signup-status` on mount and shows a friendly banner before the user fills out the form. |
| 👨‍👩‍👧 **Parent Guidance Tips** | Friendly contextual banners on parent-coordinated features — Learn to Write, Journal, Read & Quiz, and My Writing — explaining the parent's role and any known limitations (e.g. letter audio availability). |
...
| Section | Description |
|---|---|
| 📊 **Dashboard** | Usage metrics across users and children. Manual 🔄 refresh button + auto-refresh interval dropdown (1 min / 5 min / 15 min / 30 min). AI Credits this month sourced from `ai_usage_log` (never zeroed by quota reset). Signup kill switch toggle — pauses/re-opens new user registrations; card hidden until state is fetched to prevent a flash of the wrong state. |
