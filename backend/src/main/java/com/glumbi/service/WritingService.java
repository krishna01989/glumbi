...
public class WritingService {
    ...
    public Page<Map<String, Object>> getByChildPaged(Long childId, Pageable pageable) {
        Page<WritingEntry> roots = repo.findByChildIdAndSeriesIdIsNullOrderByCreatedAtDesc(childId, pageable);
        List<Map<String, Object>> content = roots.getContent().stream().map(root -> {
            List<WritingEntry> chapters = repo.findBySeriesId(root.getId());
